...
class ModelTrainer:
    ...
    def model_trainer(self, cat_df, Y_df):
        try:
            ...
            '''Grid search cv'''
            ridge_grid_search = GridSearchCV(estimator=ridge, param_grid=RIDGE_PARAMS, cv=CV)
            lasso_grid_search = GridSearchCV(estimator=lasso, param_grid=LASSO_PARAMS, cv=CV)
            polynomial_grid_search = GridSearchCV(estimator=polynomial_pipe, param_grid=POLYNOMIAL_PARAMS, cv=CV)
            random_forest_grid_search = GridSearchCV(estimator=randomforest, param_grid=RANDOM_FOREST_PARAMS, cv=CV)
            gradient_boost_grid_search = GridSearchCV(estimator=gradientboost, param_grid=GRADIENTBOOST_PARAMS, cv=CV)
            ...
            for name, model in tune_models_grid_search.items():
                train_model_grid_search[name] = self.fit_model_grid_search(model, cat_df, Y_df, model_name=name)
    ...
    def best_models(self, X, y, models):
        try:
            ...
            for key, model in models.items():
                mae_val[key] = mae(y, model.predict(X))
                print(f"{key} regression on training set MAE: {mae(y, model.predict(X)):.2f}")

            mae_val = {k: v for k, v in sorted(mae_val.items(), key=lambda item: item[1])}
    ...
    def initiate_model_trainer(self):
        try:
            ...
            df = pd.read_csv(self.data_transformation_artifact.transform_file_path)
            ...
            for model_name,_ in top_models.items():
                ...
                file_path = os.path.join(self.model_trainer_config.MODEL_TRAINER_ARTIFACT_DIR, file_name)
